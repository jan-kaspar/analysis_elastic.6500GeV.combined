...
void RunFit(TH1D *h, double t_min, double t_max, unsigned int variant, Fit::Results &results)
{
	// settings
	Fit::B_degree = 3;
	Fit::h_fit = h;
...
int main(/*int argc, char **argv*/)
{
	...
	for (unsigned int variant : {1, 2, 4})
	{
		printf("\n\n-------------------- variant %u --------------------\n", variant);

		char buf[50];
		sprintf(buf, "variant %u", variant);
		...
		// run fit
		Fit::Results fr;
		RunFit(h_in, t_min, t_max, variant, fr);
