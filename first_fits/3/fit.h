...
void AdjustBinRepresentativePoints()
{
	printf(">> Fit::AdjustBinRepresentativePoints\n");
	...
	for (int bi = bin_fit_min; bi <= bin_fit_max; ++bi)
	{
		...
		// add point to graph
		int idx = g_fit->GetN();
		g_fit->SetPoint(idx, x, y);
		g_fit->SetPointError(idx, 0., y_unc);
	}
}
...
double S2_FCN::operator() (const std::vector<double> &par) const
{
	...
	for (int i = 0; i < g_fit->GetN(); ++i)
	{
		...
		double y_unc = g_fit->GetErrorY(i);
...
unsigned int RunFit(Results &results, unsigned int modulusHighTVariant = 2)
{
	printf(">> RunFit\n");
	...
	for (unsigned int it = 0; it < N_it; it++)
	{
		...
		// adjust bin control points
		AdjustBinRepresentativePoints();
