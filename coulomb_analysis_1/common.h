...
{
	hfm->a = par[par_off_a] * 1E8;

	if (B_degree > 0) hfm->b1 = par[par_off_b + 0];
	if (B_degree > 1) hfm->b2 = par[par_off_b + 1];
	if (B_degree > 2) hfm->b3 = par[par_off_b + 2];
	if (B_degree > 3) hfm->b4 = par[par_off_b + 3];
	if (B_degree > 4) hfm->b5 = par[par_off_b + 4];
	if (B_degree > 5) hfm->b6 = par[par_off_b + 5];
	if (B_degree > 6) hfm->b7 = par[par_off_b + 6];
	if (B_degree > 7) hfm->b8 = par[par_off_b + 7];
	if (B_degree > 8) hfm->b9 = par[par_off_b + 8];

	hfm->p0 = par[par_off_p0];
	if (all_phase_parameters)
	{
		hfm->p_A = par[par_off_pAdd + 0];
		hfm->p_ka = par[par_off_pAdd + 1];
		hfm->p_tm = par[par_off_pAdd + 2];
	}
}
