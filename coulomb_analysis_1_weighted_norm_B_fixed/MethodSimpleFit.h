...
double F_fit(double mt, double par[])
{
	...
	const double norm_corr = par[par_off_norm];
...
double S2_FCN::operator() (const std::vector<double> &par) const
{
	...
	const double de_norm = (par[par_off_norm] - 1.) / 0.055;
	S2 += de_norm * de_norm;
...
unsigned int RunFit(const string & /*settings*/, Results &results)
{
	...
	unsigned int n_fit_parameters = B_degree + 3;
	...
	minuit->SetParameter(par_off_b+0, "b1", 20.7/2., 0., 0., 0.);
	...
	for (unsigned int ii = 0; ii < N_ii; ii++)
	{
		if (ii == 0)
		{
			...
			hfm->hts = 1./sqrt(minuit->GetParameter(par_off_norm));
		}
	...
	*/
	...
	g_fit_data->SetPoint(10, 0., minuit->GetParameter(par_off_norm));
	g_fit_data->SetPoint(11, 0., minuit->GetParError(par_off_norm));

	g_fit_data->SetPoint(12, 0., data_coll.size());

	const double A_p = minuit->GetParameter(par_off_norm) * cnts->sig_fac * minuit->GetParameter(par_off_a) * 1E8 * minuit->GetParameter(par_off_a) * 1E8;
